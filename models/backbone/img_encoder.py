...
@BACKBONES.register_module()
class VPTCLIPAttrPromptVisionTransformer(nn.Module):
    def __init__(self, input_resolution=512, patch_size=16, width=768, layers=12, heads=12, output_dim=512,
                 drop_path_rate=0.1, out_indices=[11], pretrained=None, get_embeddings=True,
                 num_tokens=19, prompt_dim=768, total_d_layer=11, TEPath=None, **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.input_resolution = input_resolution
        self.output_dim = output_dim
    ...
    def _init_prompt(self, patch, num_tokens, prompt_dim, total_d_layer):
        ...
        # attrPrompt = np.load('/data1/zhang_runtong/projects/AttrCLIP_transfer/configs/_base_/datasets/text_embedding/vsac_attrclip_v2.npy')
        print('tips:%s' % self.TEPath)
        attrPrompt = np.load(self.TEPath)
        attrPrompt = torch.from_numpy(attrPrompt).type(torch.cuda.FloatTensor)
        attrPrompt = attrPrompt.unsqueeze(0)
        attrPromptLength = attrPrompt.size()[2]

        if total_d_layer >= 0:
            #self.prompt_embeddings = nn.Parameter(torch.zeros(1, num_tokens, prompt_dim))
            self.prompt_embeddings = nn.Parameter(attrPrompt)
            # xavier_uniform initialization
            nn.init.uniform_(self.prompt_embeddings.data, -val, val)

            if total_d_layer > 0:  # noqa
                #self.deep_prompt_embeddings = nn.Parameter(torch.zeros(total_d_layer, num_tokens, prompt_dim))
                self.deep_prompt_embeddings = nn.Parameter(attrPrompt.repeat(total_d_layer, 1, 1))
                # xavier_uniform initialization
                nn.init.uniform_(self.deep_prompt_embeddings.data, -val, val)

            #self.prompt_proj = nn.Linear(prompt_dim, prompt_dim)
            self.prompt_proj = nn.Linear(attrPromptLength, prompt_dim)
            #self.prompt_proj2 = nn.Linear(prompt_dim, prompt_dim)
            nn.init.kaiming_normal_(self.prompt_proj.weight, a=0, mode='fan_out')
            self.prompt_norm = LayerNorm(prompt_dim, eps=1e-6)
            self.prompt_dropout = Dropout(0.1)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.total_d_layer >= 0:
            ...
            # x = torch.cat((
            #     x[:, :1, :],
            #     self.prompt_dropout(self.prompt_proj2(self.prompt_proj(self.prompt_embeddings)).expand(B, -1, -1)),
            #     x[:, 1:, :]
            # ), dim=1)
    ...
    def forward_deep_prompt(self, embedding_output, features, H, W, out_last=False):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                # deep_prompt_emb = self.prompt_dropout(
                #     self.prompt_proj2(self.prompt_proj(self.deep_prompt_embeddings[i - 1])).expand(B, -1, -1)).permute(1, 0, 2)
                deep_prompt_emb = self.prompt_dropout(
                    self.prompt_proj(self.deep_prompt_embeddings[i - 1]).expand(B, -1, -1)).permute(1, 0, 2)
                hidden_states = torch.cat((
                    hidden_states[:1, :, :],
