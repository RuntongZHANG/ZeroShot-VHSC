...
class TPN_DecoderLayer(TransformerDecoderLayer):
    def __init__(self, class_num, **kwargs):
        ...
        #self.multihead_attn = nn.Linear(512, 19, bias=False)
        self.multihead_attn = nn.Conv2d(512, class_num, (3, 3), stride=1, padding=1, bias=False)
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        n,c,p = memory.shape
        memory = memory.reshape(n, c, int(math.sqrt(p)), int(math.sqrt(p)))
        memory = self.multihead_attn(memory)
...
class ATMSingleHeadSegAttr(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(ATMSingleHeadSegAttr, self).__init__(
        ...
        for i in range(self.use_stages):
            ...
            # decoder layer
            decoder_layer = TPN_DecoderLayer(d_model=dim, nhead=nhead, dim_feedforward=dim * 4, class_num=len(self.all_idx))
    ...
    def forward(self, inputs_both, self_training=None):
        ...
        for idx, decoder_ in enumerate(self.decoder):
            _, attn_ = decoder_(None, lateral.transpose(0, 1))
            for _, attn in enumerate(attn_):
