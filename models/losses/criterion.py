...
def cross_entropy_loss(inputs, targets, bs):
    # loss_weights = torch.tensor([0.355, 2.626, 2.915, 5.607, 3.896, 8.347, 4.561, 4.420, 5.416, 7.980,
    #                              2.993, 6.559, 3.837, 4.335, 6.950, 7.547, 5.623, 5.959, 4.703, 4.010]).type(torch.cuda.FloatTensor)

    # loss_fuc = torch.nn.CrossEntropyLoss(weight=loss_weights)
    loss_fuc = torch.nn.CrossEntropyLoss()
    ce_loss = loss_fuc(inputs, targets)
...
class SegPlusCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        ### process for ce_loss
        # target_masks_ce = torch.empty((bs, H, W)).type(torch.cuda.LongTensor)
        # for idx, item in enumerate(targets):
        #     label = item['labels'] + 1
        #     target_mask_float = item['target_masks'].type(torch.cuda.LongTensor)
        #     tmp = label[:, None, None] * target_mask_float
        #     target_masks_ce[idx] = tmp.sum(0).type(torch.cuda.LongTensor)
        #     # outs = torch.unique(target_masks_ce[idx], return_counts=True)
        #     # print(outs)
        # losses = {
        #     "loss_mask": cross_entropy_loss(outputs["pred_masks"], target_masks_ce, bs),
        #     "loss_dice": dice_loss(src_masks_dice, target_masks_dice, num_masks),
        # }
        return losses
...
class SegOursCriterion(nn.Module):
    # in this version, both all masks and logits will be added to compute loss
    """This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
